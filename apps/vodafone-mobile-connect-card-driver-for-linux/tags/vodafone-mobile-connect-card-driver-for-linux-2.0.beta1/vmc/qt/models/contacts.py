...
class ContactsStoreModel(TreeStoreModel):
    ...
    def add_contacts(self, contacts):
        ...
        contacts = [c for c in contacts if not is_sim_contact(c)]
        for contact in contacts:
            self.add_contact(contact)
    
    def add_contact(self, contact):
        """Adds C{contact} to the store"""
        if not is_sim_contact(contact):
            if contact.group in self.groupIters:
                groupIter = self.groupIters[contact.group]
            else:
                groupIter = self.append(None, [None, contact.group, '', '', None])
                self.groupIters[contact.group] = groupIter
