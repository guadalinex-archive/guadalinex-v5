...
The csv file that you have tried to import has an invalid format.""")
    ...
    def delete_entries(self, menuitem, pathlist, treeview):
        ...
        def delete_contacts_cb(contacts):
            d_contacts = {}
            for contact in contacts:
                d_contacts[contact.number] = d_contacts.get(contact.number, []) + [contact]
            for obj in objs:
                if obj:
                    # Get related contacts
                    _objs = d_contacts.get(obj.number, [])
                    for _obj in _objs:
                        # Ask confirmation for sim contacts
                        msg = _('Delete SIM entry?')
                        desc = _('Do you want to delete the SIM entry for "%s" with number "%s"?' % (_obj.name, _obj.number))
                        if not is_sim_contact(_obj) or dialogs.open_warning_request_cancel_ok(msg, desc):
                            manager.delete_contact(_obj)

        contacts_tree = (treeview.get_name() == 'contacts_treeview')
        if contacts_tree:
            manager = get_phonebook(self.model.get_sconn())
            d = manager.get_contacts()
            d.addCallback(delete_contacts_cb)
        else:
