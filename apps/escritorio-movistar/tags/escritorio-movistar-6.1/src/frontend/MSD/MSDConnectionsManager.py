...
STATUS_MESSAGES = ["Desconectado","Conectado","Conectando","Desconectando"]
BUTTON_TITLES = ["Conectar","Desconectar","Conectar","Desconectar"]
...
class MSDConnectionsManager:
    ...
    def __async_init_status_error_cb(self, data=None):
        print "error in MSDConnectionsManager in async status"
    
    def set_cardmanager(self, cardmanager):
        self.cardmanager = None 
    ...
    def connect_to_bus (self):
        ...
        if self.mcontroller.dialer != None :
            ...
            print "No se ha encontrado el ppp manager"
            self.ppp_manager = None
    ...
    def __is_device_selected (self):
        print "MSDConnManager : method -> __is_device_selected()"
        dev = self.mcontroller.get_active_device()
	if dev != None :
		return True
	else:
    ...
    def change_bar_status(self,new_status=None):
        ...
        print "actualizando ui para %s" % status
    ...
    def __abort_connection_actions(self):
        ...
        self.main_stats.set_connection_name(value="Ninguna establecida")
    ...
    def __connected_cb(self, dialer):
        ...
        self.starting_connection = None
        self.launch_service()
        print "CONNECTED ---> %s" % self.actual_connection
    ...
    def __connecting_cb(self, dialer):
        ...
        if self.reconnecting_flag == False:
            self.progress_dialog.show(title=None, message="Conectando con '%s'" %  self.starting_connection,
                                      cancel_callback=self.__abort_connection_now)
        print "CONNECTING ---> %s" % self.starting_connection
        self.connection_successful = False
        self.reconnecting_flag = False
    ...
    def __disconnected_cb(self, dialer):
        ...
        self.connect_with_phone = None
        print "DISCONNECTED ---> %s" % self.actual_connection
    ...
    def __disconnecting_cb(self, dialer):
        ...
        if self.connection_successful == True:
            self.__set_sensitive_connect_disconnect_buttons(False)
            if self.waiting_connection == None:
                self.progress_dialog.show(title=None, message="Desconectando '%s'" %  self.actual_connection,
                                          cancel_callback=self.__abort_connection_now)
            else:
                self.progress_dialog.set_show_buttons(True)
                self.progress_dialog.show(title=None, message="Conectando a '%s'" %  self.waiting_connection,
                                          cancel_callback=self.__abort_connection_now)
                self.reconnecting_flag = True
        else:
            self.progress_dialog.hide()
        
        print "DISCONNECTING ---> %s" % self.actual_connection
    ...
    def __detect_active_connection_dialog(self, connection_name, action, bookmark_info):
        ...
        vbox = dlg.vbox
        check_button = gtk.CheckButton("No volver a mostrar este dialogo")
        ...
        dlg.add_buttons(gtk.STOCK_CANCEL, 1, u"_Usar conexión establecida", 2, gtk.STOCK_CONNECT, 3)                  
        ret = dlg.run()
        dlg.destroy()
    ...
    def __new_connection_active_dialog(self, connection_name, action, bookmark_info):
        ...
        dlg.set_icon_from_file(MSD.icons_files_dir + "main_window_icon.png")
        vbox = dlg.vbox
        check_button = gtk.CheckButton("No preguntar de nuevo")
        vbox.pack_end(check_button, expand=False)
