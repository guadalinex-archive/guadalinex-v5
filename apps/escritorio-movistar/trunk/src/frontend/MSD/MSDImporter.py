...
class MSDImporter:
    ...
    def __add_bookmark_to_conf(self, bookmark, conn=None):
        ...
        if bookmark["uri"] == None:
            ...
            mesg.set_markup(_(u"<b>Este acceso directo no puede ser importado</b>"))
            mesg.format_secondary_markup(_("Error al importar el acceso directo. Probablemente el archivo asociado no exista en tu sistema."))
        ...
        if not self.conf.exists_connection(conn["name"]) :
            if conn["user"] != None:
                ...
                if ret != gtk.RESPONSE_OK:
                    print _("Importacion cancelada")
                    return
                
                self.__add_connection_to_conf(conn)
            else:
                print _("Caso de que no haya usuario")
                ...
                print repr(conn["name"])
                print "---------------------------------------"
                self.imp_connection_label.set_markup(_(u"<b>Conexión : '%s'</b>") % conn["name"])
            ...
            if conn["name"] == "movistar Internet" or conn["name"] == "movistar Internet directo" :
                ...
                mesg.set_markup(_(u"<b>La conexión '%s' no se puede sobrescribir</b>") % conn["name"])
                if (mesg.run() != None):
                    mesg.destroy()
                    return
            else:
                print _("preguntar la conexion, si desea ser sobreescrita")
                if conn["user"] != None:
    ...
    def __add_bookmark(self, dict, bookmark):
        ...
        if not self.conf.exists_bookmark(bookmark["name"]) :
            ...
            self.imp_filename_label.set_markup(u"%s" % self.file)
            self.imp_bookmark_label.set_markup(_(u"<b>Acceso directo : '%s'</b>") % bookmark["name"])
            ...
            print _("preguntar si se puede sobreescribir el acceso directo")
    ...
    def __add_bookmark_and_connection(self, dict, bookmark, conn):
        ...
        if not self.conf.exists_bookmark(bookmark["name"]) and not self.conf.exists_connection(conn["name"]):
            ...
            if self.conf.exists_bookmark(bookmark["name"]) and self.conf.exists_connection(conn["name"]):
                if conn["name"] == "movistar Internet" or conn["name"] == "movistar Internet directo" :
                    print _("ask(Puedo sobreescribir bookmark y conn no porque es internt/gprs)")
                    ...
                    if conn["user"] != None:
                        print _("ask(Puedo sobreescribir bookmark y conn)")
                        ...
                        print _("ask(Puedo sobreescribir bookmark y conn, sin user)")
                ...
                if self.conf.exists_bookmark(bookmark["name"]):
                    print _("ask (Puedo sobreescribir bookmark)")
                    if conn["user"] != None:
                    ...
                    if conn["name"] == "movistar Internet" or conn["name"] == "movistar Internet directo" :
                        print _("ask (Puedo no sobreescribir porque es internet gprs3g )")
                        ...
                        print _("ask (Puedo sobreescribir conn)")
                        if conn["user"] != None:
    ...
    def __import_bookmark(self, dict):
        ...
        for field in needed_fields:
            if dict.has_key(field) == False:
                self.__error_importing(_("Falta el campo %s en el fichero") % field)
                return None
    ...
    def __error_importing(self, msg):
        print _("Error importando : %s") % msg
