...
class MSDAction:
    ...
    def __remove_service_button_cb (self, widget, data):
        ...
        dlg.set_title("Desinstalar servicio")
        dlg.set_markup(u"<b>¿Deseas desinstalar el servicio '%s'?</b>" % self.get_visible_action_name())
        dlg.format_secondary_markup(u"Podrás volver a instalarlo desde esta ventana de configuración.")
        ret = dlg.run()

        if ret == gtk.RESPONSE_OK :
            dlg.destroy()
            self.progress_dialog.show("Desinstalando servicio", "Por favor, espera mientras se desinstala el servicio '%s'." % self.get_conf_key_value("name"))
    ...
    def __install_service_button_cb(self, widget, data):
        self.progress_dialog.show("Instalando servicio", "Por favor, espera mientras se instala el servicio '%s'." % self.get_conf_key_value("name"))
        self.timer_id = gobject.timeout_add(2000, self.__progress_timer_cb)
        self.conf_container.show()
        self.connection_vbox.show()
        self.install_service_alignment.hide()
