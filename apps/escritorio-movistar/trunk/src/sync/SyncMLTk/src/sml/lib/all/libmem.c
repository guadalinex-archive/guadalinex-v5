...
#include <windows.h>
//typedef void *HANDLE;
static HANDLE g_smlHeap = NULL;
SML_API void smlSetHeap(void *pHeap)
{
	g_smlHeap = *(HANDLE *)pHeap;
}
SML_API void smlResetHeap()
{
	g_smlHeap = NULL;
}
#endif
...
SML_API void *smlLibRealloc(void *pObject, MemSize_t constSize)
{
  ...
  return sysync_realloc(pObject, constSize);
	...
	if (g_smlHeap!=NULL)
		if (pObject==NULL)
			return HeapAlloc(g_smlHeap, 0, constSize);
		else
			return HeapReAlloc(g_smlHeap, 0, pObject, constSize);
#endif
...
#ifdef _WIN32
	if (g_smlHeap!=NULL)
		return HeapAlloc(g_smlHeap, 0, size);
#endif
