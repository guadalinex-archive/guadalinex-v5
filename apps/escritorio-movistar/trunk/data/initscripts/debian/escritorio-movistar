...
DAEMON=/usr/sbin/ppphelper
NAME="escritorio-movistar"
DESC="escritorio-movistar (ppphelper)"

test -x $DAEMON || exit 0

# Include escritorio-movistar defaults if available
if [ -f /etc/default/escritorio-movistar ] ; then
	. /etc/default/escritorio-movistar
fi
  ...
  stop)
	if [ -e /var/run/$NAME.pid ] ;
        then
            ...
            test -e /var/run/$NAME.pid && rm /var/run/$NAME.pid
            killall -9 ppphelper 2> /dev/null || echo "No ppphelper killed"
        fi
	;;
